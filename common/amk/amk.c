...
void amkInit(amk_motor_t* motor, bool* pchg_complete, uint8_t id) {
    *motor = (amk_motor_t) {.state = AMK_STATE_OFF,
...
static void amkSendSetpoints(amk_motor_t* motor) {
    if (INVA_ID == motor->id) {
        SEND_INVA_SET(motor->control.AMK_bReserve1,
                      ...
                      motor->torque_set_ppt_nom,
                      motor->torque_lim_pos_ppt_nom,
                      motor->torque_lim_neg_ppt_nom);
        motor->control.AMK_bErrorReset = false;
        SEND_INVA_LOG_SET(motor->control.AMK_bReserve1,
                          motor->control.AMK_bInverterOn,
                          motor->control.AMK_bDcOn,
                          motor->control.AMK_bEnable,
                          motor->control.AMK_bErrorReset,
        ...
        SEND_INVB_SET(motor->control.AMK_bReserve1,
                      motor->control.AMK_bInverterOn,
                      motor->control.AMK_bDcOn,
                      motor->control.AMK_bEnable,
                      motor->control.AMK_bErrorReset,
                      ...
                      motor->torque_lim_pos_ppt_nom,
                      motor->torque_lim_neg_ppt_nom);
        SEND_INVB_LOG_SET(motor->control.AMK_bReserve1,
                          motor->control.AMK_bInverterOn,
                          motor->control.AMK_bDcOn,
                          motor->control.AMK_bEnable,
                          motor->control.AMK_bErrorReset,
...
void amkSetTorque(amk_motor_t* motor, int16_t torque_setpoint) {
    ...
    if (torque_setpoint > MAX_POSITIVE_TORQUE_SETPOINT || torque_setpoint < MAX_NEGATIVE_TORQUE_SETPOINT) {
        return;
    }
...
static void amkGetData(amk_motor_t* motor) {
    if (INVA_ID == motor->id) {
        if (can_data.INVA_CRIT.stale || can_data.INVA_TEMPS.stale || can_data.INVA_INFO.stale
            || can_data.INVA_ERR_1.stale || can_data.INVA_ERR_2.stale) {
        ...
        if (can_data.INVB_CRIT.stale || can_data.INVB_TEMPS.stale || can_data.INVB_INFO.stale
            || can_data.INVB_ERR_1.stale || can_data.INVB_ERR_2.stale) {
            setFault(motor->stale_fault_id, true);
        } else {
