...
gen_auto_eeprom_init_start = "BEGIN AUTO EEPROM INIT"
gen_auto_eeprom_init_stop  = "END AUTO EEPROM INIT"
...
def configure_node(node_config, node_paths):
    ...
    if 'files' in node_config:
        for file in node_config['files']:
            file_struct_lines.append("typedef struct { __attribute__((packed))\n")
            for file_var in file['contents']:
                file_struct_lines.append(f"    {file_var['type']}   {file_var['var_name']};\n")
            file_struct_lines.append(f"}} {file['name']}_t;\n\n")
    ...
    if 'files' in node_config:
        for file in node_config['files']:
            file_map_lines.append(f"    mapMem(&{file['name']}, sizeof({file['name']}), \"{file['eeprom_lbl']}\", 1);\n")
    generator.insert_lines(c_lines, gen_auto_eeprom_init_start, gen_auto_eeprom_init_stop, file_map_lines)
