...
bool PHAL_initADC(ADC_TypeDef* adc, ADCInitConfig_t* config, ADCChannelConfig_t channels[], uint8_t num_channels) {
    ...
    for (int i = 0; i < num_channels; i++) {
        ...
        if (channels[i].channel < 10) {
            ...
            adc->SMPR2 |= (channels[i].sampling_time & ADC_SMPR2_SMP0_Msk)
                << (ADC_SMPR2_SMP1_Pos * channels[i].channel);
            ...
            adc->SMPR1 |= (channels[i].sampling_time & ADC_SMPR1_SMP10_Msk)
                << (ADC_SMPR1_SMP11_Pos * (channels[i].channel - 10));
        }
