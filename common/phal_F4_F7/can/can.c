...
bool PHAL_txCANMessage(CanMsgTypeDef_t* msg, uint8_t txMbox) {
    ...
    msg->Bus->sTxMailBox[txMbox].TDLR = ((uint32_t)msg->Data[3] << 24) | ((uint32_t)msg->Data[2] << 16)
        | ((uint32_t)msg->Data[1] << 8) | ((uint32_t)msg->Data[0]);
    msg->Bus->sTxMailBox[txMbox].TDHR = ((uint32_t)msg->Data[7] << 24) | ((uint32_t)msg->Data[6] << 16)
