...
bool PHAL_initDMA(dma_init_t* init) {
    ...
    init->stream->CR |= (init->dma_chan_request << DMA_SxCR_CHSEL_Pos) | (init->priority << DMA_SxCR_PL_Pos)
        | (init->mem_size << DMA_SxCR_MSIZE_Pos);

    // Set peripheral data size, memory increment, peripheral increment
    init->stream->CR |= (init->periph_size << DMA_SxCR_PSIZE_Pos) | (init->mem_inc << DMA_SxCR_MINC_Pos)
        | (init->periph_inc << DMA_SxCR_PINC_Pos);

    // Set circular mode, direction, transfer error interrupt enable, and transfer complete interrupt enable
    init->stream->CR |= (init->circular << DMA_SxCR_CIRC_Pos) | (init->dir << DMA_SxCR_DIR_Pos)
        | (init->tx_isr_en << DMA_SxCR_TCIE_Pos) | (init->tx_isr_en << DMA_SxCR_TEIE_Pos);
