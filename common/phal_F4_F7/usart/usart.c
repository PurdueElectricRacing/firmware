...
static void handleUsartIRQ(USART_TypeDef* handle, uint8_t idx) {
    ...
    if (sr & USART_SR_CTS) {
        sr &=
            ~USART_SR_CTS; // We currently do not plan on using CTS or LIN mode in the near future, so this is placeholder code for the future
    }
    // Lin Break Detection flag
    if (sr & USART_SR_LBD) {
        sr &=
            ~USART_SR_LBD; // We currently do not plan on using CTS or LIN mode in the near future, so this is placeholder code for the future
    }
    // Overrun Error Flag
    if (sr & USART_SR_ORE) {
...
//NOTE: According to RM0090, it is not safe to clear the TC bit unless Multibuffer mode is enabled.
    ...
    // Clear to send flag
    if (sr & USART_ISR_CTS) {
        active_uarts[idx].active_handle->periph->ICR |=
            USART_ICR_CMCF; // We currently do not plan on using CTS or LIN mode in the near future, so this is placeholder code for the future
    }
    // Lin Break Detection flag
    if (sr & USART_ISR_LBDF) {
        active_uarts[idx].active_handle->periph->ICR |=
            USART_ICR_LBDCF; // We currently do not plan on using CTS or LIN mode in the near future, so this is placeholder code for the future
    }
    // Overrun Error Flag
    if (sr & USART_ISR_ORE) {
        // Communicate these errors to the user, so we can clear this bit but leave USART clear for future transactions
...
static void handleDMAxComplete(uint8_t idx, uint32_t irq, uint8_t dma_type) {
    ...
    if (dma_type == USART_DMA_TX) {
        ...
        sr_reg = (dma_num <= 3) ? &active_uarts[idx].active_handle->tx_dma_cfg->periph->LISR
                                : &active_uarts[idx].active_handle->tx_dma_cfg->periph->HISR;
        csr_reg = (dma_num <= 3) ? &active_uarts[idx].active_handle->tx_dma_cfg->periph->LIFCR
                                 : &active_uarts[idx].active_handle->tx_dma_cfg->periph->HIFCR;
    } else // DMA RX is ongoing
    {
        dma_num = active_uarts[idx].active_handle->rx_dma_cfg->stream_idx;

        // Select appropriate flag and clear registers for Stream
        sr_reg = (dma_num <= 3) ? &active_uarts[idx].active_handle->rx_dma_cfg->periph->LISR
                                : &active_uarts[idx].active_handle->rx_dma_cfg->periph->HISR;
        csr_reg = (dma_num <= 3) ? &active_uarts[idx].active_handle->rx_dma_cfg->periph->LIFCR
                                 : &active_uarts[idx].active_handle->rx_dma_cfg->periph->HIFCR;
    }
