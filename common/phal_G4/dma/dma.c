...
bool PHAL_initDMA(dma_init_t* dma) {
    ...
    dma->channel->CCR |= (dma->mem_size << DMA_CCR_MSIZE_Pos) & DMA_CCR_MSIZE_Msk
        ...
        | (dma->priority << DMA_CCR_PL_Pos) & DMA_CCR_PL_Msk | (dma->mem_inc << DMA_CCR_MINC_Pos) & DMA_CCR_MINC_Msk
        | (dma->periph_inc << DMA_CCR_PINC_Pos) & DMA_CCR_PINC_Msk
        | (dma->circular << DMA_CCR_CIRC_Pos) & DMA_CCR_CIRC_Msk | (dma->dir << DMA_CCR_DIR_Pos) & DMA_CCR_DIR_Msk
