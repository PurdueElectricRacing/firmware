...
bool PHAL_FDCAN_init(FDCAN_GlobalTypeDef *fdcan, bool loopback, uint32_t bit_rate) {
    uint32_t nbtp;
    if (!PHAL_FDCAN_priv_getNBTP(bit_rate, &nbtp))
        return false;
    ...
    // Classic CAN only: no FD, no bit-rate switching.
    ...
    fdcan->NBTP  = nbtp;
    fdcan->TXBC &= ~FDCAN_TXBC_TFQM; // FIFO mode (not queue) for the TX buffer

    // Route RX-new-message to line 0, TX-complete to line 1.
    ...
    // Default policy until PHAL_FDCAN_setFilters narrows it: accept everything.
...
bool PHAL_FDCAN_setFilters(FDCAN_GlobalTypeDef *fdcan,
                           ...
                           uint32_t *xid_list,
                           uint32_t num_xid) {
    if (num_sid > MAX_NUM_SID_FILTER || num_xid > MAX_NUM_XID_FILTER)
        return false;

    PHAL_FDCAN_priv_enterConfig(fdcan);

    // Anything not matched by a filter below is rejected.
