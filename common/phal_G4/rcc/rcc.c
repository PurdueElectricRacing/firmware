...
uint8_t PHAL_configureClockRates(ClockRateConfig_t* config) {
    ...
    /* Configure PLL */
    if (config->use_pll) {
        ret_code |= (!PHAL_configurePLLVCO(config->pll_src, config->vco_output_rate_target_hz))
...
bool PHAL_configurePLLVCO(PLLSrc_t pll_source, uint32_t vco_output_rate_target_hz) {
    ...
    vco_output_rate_target_hz =
        vco_output_rate_target_hz > RCC_MAX_VCO_RATE_HZ ? RCC_MAX_VCO_RATE_HZ : vco_output_rate_target_hz;
    vco_output_rate_target_hz =
        vco_output_rate_target_hz < RCC_MIN_VCO_RATE_HZ ? RCC_MIN_VCO_RATE_HZ : vco_output_rate_target_hz;

    // Turn off and wait for PLL to disable
    RCC->CR &= ~RCC_CR_PLLON;
    while ((RCC->CR & RCC_CR_PLLRDY))
        ; // Wait for PLL to turn off
    RCC->PLLCFGR &= ~(RCC_PLLCFGR_PLLSRC_Msk | RCC_PLLCFGR_PLLN_Msk
                      | RCC_PLLCFGR_PLLM_Msk); // Clear any PLL source, N, and M configuration
    ...
    for (; pll_input_divisor <= RCC_MAX_PLL_INPUT_DIVISOR; pll_input_divisor++) // PLLM must be 2 to 63 (Pg. 227)
    {
        ...
        if (pll_vco_in_rate < 1000000
            || pll_vco_in_rate > 2000000) // VCO input rate must be 1MHz to 2MHz (Pg. 227 PLLM)
        {
            continue;
        }

        // VCO output frequency = VCO input * PLLN
        for (; pll_output_multiplier <= RCC_MAX_PLL_OUTPUT_MULTIPLIER;
             pll_output_multiplier++) // PLLN must be 50 to 432 (Pg. 227)
        {
...
bool PHAL_configureHSISystemClock() {
    ...
    while ((RCC->CFGR & RCC_CFGR_SWS_HSI)
           != RCC_CFGR_SWS_HSI) // Wait until the system clock switch register indicates that HSI is selected
        ;
    __DSB(); // Wait for explicit memory accesses to finish
...
bool PHAL_configureHSESystemClock() {
    ...
    while ((RCC->CFGR & RCC_CFGR_SWS_HSE)
           != RCC_CFGR_SWS_HSE) // Wait until the system clock switch register indicates that HSE is selected
        ;
    __DSB(); // Wait for explicit memory accesses to finish
