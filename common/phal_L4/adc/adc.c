...
#error "STM32 Arch not currently supported for ADC"
    ...
    if (config->dma_mode != ADC_DMA_OFF) {
        adc->CFGR |=
            (ADC_CFGR_DMAEN) | (((config->dma_mode == ADC_DMA_CIRCULAR) << ADC_CFGR_DMACFG_Pos) & ADC_CFGR_DMACFG);
    } else
    ...
    for (int i = 0; i < num_channels; i++) {
        ...
        if (channels[i].channel < 10) {
            ...
            adc->SMPR1 |= (channels[i].sampling_time & ADC_SMPR1_SMP0_Msk)
                << (ADC_SMPR1_SMP1_Pos * channels[i].channel);
            ...
            adc->SMPR2 |= (channels[i].sampling_time & ADC_SMPR2_SMP10_Msk)
                << (ADC_SMPR2_SMP11_Pos * (channels[i].channel - 10));
        }
