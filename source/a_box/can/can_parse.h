...
#define SEND_IMD_RESISTANCE(Condition_, Resistance_) do {\
        CanMsgTypeDef_t msg = {.Bus=CAN1, .ExtId=ID_IMD_RESISTANCE, .DLC=DLC_IMD_RESISTANCE, .IDE=1};\
        CanParsedData_t* data_a = (CanParsedData_t *) &msg.Data;\
        data_a->imd_resistance.Condition = Condition_;\
        data_a->imd_resistance.Resistance = FLOAT_TO_UINT32(Resistance_);\
        canTxSendToBack(&msg);\
    } while(0)
