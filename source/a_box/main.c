...
dma_init_t adc_dma_config =
    ADC1_DMA_CONT_CONFIG((uint32_t)&adc_readings, sizeof(adc_readings) / sizeof(adc_readings.tmu_1_1), 0b01);
...
void heartBeatLED() {
    ...
    static uint8_t trig;
    if (trig)
        SEND_A_BOX_CAN_STATS(can_stats.can_peripheral_stats[CAN1_IDX].tx_of,
