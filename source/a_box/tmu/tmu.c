...
bool initTMU(tmu_handle_t *tmu)
{
   ...
   PHAL_SPI_transfer_noDMA(tmu->spi, spi_tx_buffer, 1, 3, spi_rx_buffer);
   while (PHAL_SPI_busy(tmu->spi))
       ;
...
void readTemps(tmu_handle_t *tmu) {
   ...
   uint8_t spi_rx_buffer[11] = {0};
    ...
    while(PHAL_SPI_busy(tmu->spi))
        ;
    spi_tx_buffer[0] = (0x5u << 2) | (0x1);
    //spi_tx_buffer[0] = (0x5u) | (0x1);
    PHAL_SPI_transfer_noDMA(tmu->spi, spi_tx_buffer, 1, 11, spi_rx_buffer);
    while (PHAL_SPI_busy(tmu->spi))
        ;
