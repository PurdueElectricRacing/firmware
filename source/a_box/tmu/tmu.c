...
uint8_t readTemps() {
    ...
    for (uint8_t module = 0; module < NUM_MODULES; module++) {
        ...
        left_readings->temp_readings[curr_therm] = left_raw_adc_reading > ADC_ERROR_HIGH ? ERROR_HIGH
            : left_raw_adc_reading < ADC_ERROR_LOW                                       ? ERROR_LOW
                                                   : ADC_to_temp[left_raw_adc_reading - ADC_ERROR_LOW];
        right_readings->temp_readings[curr_therm] = right_raw_adc_reading > ADC_ERROR_HIGH ? ERROR_HIGH
            : right_raw_adc_reading < ADC_ERROR_LOW                                        ? ERROR_LOW
    ...
    SEND_RAW_CELL_TEMP_MODULE1(curr_therm,
                               module_one->left_readings.temp_readings[curr_therm],
                               module_one->left_readings.temp_readings[curr_therm]);
    SEND_RAW_CELL_TEMP_MODULE2(curr_therm,
                               module_two->left_readings.temp_readings[curr_therm],
                               module_two->left_readings.temp_readings[curr_therm]);
    SEND_RAW_CELL_TEMP_MODULE3(curr_therm,
                               module_three->left_readings.temp_readings[curr_therm],
                               module_three->left_readings.temp_readings[curr_therm]);
    SEND_RAW_CELL_TEMP_MODULE4(curr_therm,
                               module_four->left_readings.temp_readings[curr_therm],
                               module_four->left_readings.temp_readings[curr_therm]);
    SEND_RAW_CELL_TEMP_MODULE5(curr_therm,
                               module_five->left_readings.temp_readings[curr_therm],
                               module_five->left_readings.temp_readings[curr_therm]);
    ...
    if (curr_therm < (NUM_THERM - 1)) {
        ...
        for (uint8_t module = 0; module < NUM_MODULES; module++) {
            ...
            tmu.module_temps[module].total_avg_temp =
                (left_readings->avg_temp + right_readings->avg_temp) / THERM_PER_MODULE;
        }
        ...
        SEND_MOD_CELL_TEMP_AVG_A_B_C(module_one->total_avg_temp,
                                     module_two->total_avg_temp,
                                     module_three->total_avg_temp);
        SEND_MOD_CELL_TEMP_AVG_D_E(module_four->total_avg_temp, module_five->total_avg_temp);
        SEND_MOD_CELL_TEMP_MAX_A_B_C(module_one->total_max_temp,
                                     module_two->total_max_temp,
                                     module_three->total_max_temp);
        SEND_MOD_CELL_TEMP_MAX_D_E(module_four->total_max_temp, module_five->total_max_temp);
        SEND_MOD_CELL_TEMP_MIN_A_B_C(module_one->total_min_temp,
                                     module_two->total_min_temp,
                                     module_three->total_min_temp);
        SEND_MOD_CELL_TEMP_MIN_D_E(module_four->total_min_temp, module_five->total_min_temp);
        int16_t max_temp = MAX(MAX(MAX(module_one->total_max_temp, module_two->total_max_temp),
                                   MAX(module_three->total_max_temp, module_four->total_max_temp)),
                               module_five->total_max_temp);
        int16_t min_temp = MIN(MIN(MIN(module_one->total_min_temp, module_two->total_min_temp),
                                   MIN(module_three->total_min_temp, module_four->total_min_temp)),
                               module_five->total_min_temp);
        SEND_MAX_CELL_TEMP(max_temp);
        SEND_NUM_THERM_BAD(module_one->left_readings.num_bad,
                           module_one->left_readings.num_bad,
                           module_two->left_readings.num_bad,
                           module_two->left_readings.num_bad,
                           module_three->left_readings.num_bad,
                           module_three->left_readings.num_bad,
                           module_four->left_readings.num_bad,
                           module_four->left_readings.num_bad,
                           module_five->left_readings.num_bad,
                           module_five->left_readings.num_bad);
