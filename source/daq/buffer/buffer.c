...
int8_t bSendToBack(b_handle_t* b, const void* data) {
    ...
    for (uint32_t i = 0; i < b->_item_size; ++i)
...
uint32_t bGetItemCount(b_handle_t* b, uint8_t t_idx) {
    ...
    if (t->_tail <= head)
        ...
        return b->_max_items - (t->_tail - head);
}
...
uint32_t bGetFillLevel(b_handle_t* b) {
    ...
    if (!b->_min_tail_active)
        return 0;
    if (b->_min_tail <= head)
        return head - b->_min_tail;
    else
        return b->_max_items - (b->_min_tail - head);
}
...
int8_t bGetTailForRead(b_handle_t* b, uint8_t t_idx, void** rx_loc, uint32_t* contiguous_items) {
    ...
    if (t->_tail <= head)
        *contiguous_items = head - t->_tail;
    else
        *contiguous_items = b->_max_items - t->_tail;
    if (*contiguous_items == 0)
        return -1;
...
int8_t bCommitRead(b_handle_t* b, uint8_t t_idx, uint32_t items_read) {
    ...
    if (t->_tail <= head)
        ...
        max_contig_items = b->_max_items - t->_tail;
