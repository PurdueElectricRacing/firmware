...
static void daq_heartbeat(void) {
    ...
    SEND_DAQ_CAN_STATS(can_stats[BUS_ID_CAN1].tx_of,
                       can_stats[BUS_ID_CAN1].tx_fail,
                       can_stats[BUS_ID_CAN1].rx_of,
                       can_stats[BUS_ID_CAN1].rx_overrun);
    if (dh.bcan_rx_overflow || dh.can1_rx_overflow || dh.sd_rx_overflow || dh.tcp_tx_overflow) {
        SEND_DAQ_QUEUE_STATS(dh.bcan_rx_overflow,
                             dh.can1_rx_overflow,
                             dh.sd_rx_overflow,
                             dh.tcp_tx_overflow); // TODO reset & only send once?
    }
}
...
void daq_shutdown_hook(void) {
    ...
    // all LEDs go bye bye
    GPIOD->BSRR |=
        GPIO_CLEAR_BIT(HEARTBEAT_LED_PIN) | GPIO_CLEAR_BIT(CONNECTION_LED_PIN) | GPIO_CLEAR_BIT(ERROR_LED_PIN);
    GPIOA->BSRR |=
        GPIO_CLEAR_BIT(SD_DETECT_LED_PIN) | GPIO_CLEAR_BIT(SD_ACTIVITY_LED_PIN) | GPIO_CLEAR_BIT(SD_ERROR_LED_PIN);
