...
void PHAL_SDIO_SendCommand(PHAL_SD_Cmd_t* cmd) {
    SDIO->ARG = cmd->arg;
    SDIO->CMD = ((cmd->idx & SDIO_CMD_CMDINDEX_Msk)) | ((cmd->res << SDIO_CMD_WAITRESP_Pos) & SDIO_CMD_WAITRESP_Msk)
        | SDIO_CMD_CPSMEN;
}
...
SD_Error SD_ReadMultiBlocks(uint8_t* readbuff, uint64_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks) {
    ...
    SDIO->MASK |= (SDIO_MASK_DCRCFAILIE | SDIO_MASK_DTIMEOUTIE | SDIO_MASK_DATAENDIE | SDIO_MASK_RXOVERRIE
...
SD_Error SD_WriteMultiBlocks(uint8_t* writebuff, uint64_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks) {
    ...
    SDIO->MASK |= (SDIO_MASK_DCRCFAILIE | SDIO_MASK_DTIMEOUTIE | SDIO_MASK_DATAENDIE | SDIO_MASK_TXUNDERRIE
                   | SDIO_MASK_STBITERRIE);
...
static SD_Error FindSCR(uint16_t rca, uint32_t* pscr) {
    ...
    while (!(SDIO->STA
             & (SDIO_STA_RXOVERR | SDIO_STA_DCRCFAIL | SDIO_STA_DTIMEOUT | SDIO_STA_DBCKEND | SDIO_STA_STBITERR))) {
...
void SDIO_IRQHandler(void) {
    ...
    SDIO->MASK &= ~(SDIO_MASK_DCRCFAILIE | SDIO_MASK_DTIMEOUTIE | SDIO_MASK_DATAENDIE | SDIO_MASK_TXFIFOHEIE
                    | SDIO_MASK_RXFIFOHFIE | SDIO_MASK_TXUNDERRIE | SDIO_MASK_RXOVERRIE | SDIO_MASK_STBITERRIE);
}
