...
GPIOInitConfig_t gpio_config[] = {
 ...
 // LCD
 // TODO: once uart merged
 // Dashboard Input
 GPIO_INIT_INPUT(DAQ_SW_GPIO_Port, DAQ_SW_Pin, GPIO_INPUT_OPEN_DRAIN),
 ...
 // LV Status
 GPIO_INIT_ANALOG(LV_24V_SENSE_GPIO_Port, LV_24V_SENSE_Pin),
 GPIO_INIT_ANALOG(LV_12V_SENSE_GPIO_Port, LV_12V_SENSE_Pin),
 GPIO_INIT_ANALOG(LV_5V_SENSE_GPIO_Port, LV_5V_SENSE_Pin),
 GPIO_INIT_ANALOG(LV_3V3_SENSE_GPIO_Port, LV_3V3_SENSE_Pin),
};
...
ADCChannelConfig_t adc_channel_config[] = {
   ...
   {.channel=LV_24V_SENSE_ADC_CHNL, .rank=9,  .sampling_time=ADC_CHN_SMP_CYCLES_480},
   {.channel=LV_12V_SENSE_ADC_CHNL, .rank=10, .sampling_time=ADC_CHN_SMP_CYCLES_480},
   {.channel=LV_5V_SENSE_ADC_CHNL,  .rank=11, .sampling_time=ADC_CHN_SMP_CYCLES_480},
   {.channel=LV_3V3_SENSE_ADC_CHNL, .rank=12, .sampling_time=ADC_CHN_SMP_CYCLES_480},
   {.channel=MCU_THERM_ADC_CHNL,    .rank=13, .sampling_time=ADC_CHN_SMP_CYCLES_480}
};
...
void preflightAnimation(void) {
    ...
    bool on = time++ % 2;
    PHAL_writeGPIO(BMS_LED_GPIO_Port, BMS_LED_Pin, on);
    PHAL_writeGPIO(IMD_LED_GPIO_Port, IMD_LED_Pin, on);
    PHAL_writeGPIO(PRCHG_LED_GPIO_Port, PRCHG_LED_Pin, on);

    PHAL_writeGPIO(CONN_LED_GPIO_Port, CONN_LED_Pin, on);
    PHAL_writeGPIO(HEART_LED_GPIO_Port, HEART_LED_Pin, on);
    PHAL_writeGPIO(ERROR_LED_GPIO_Port, ERROR_LED_Pin, on);
...
{    
    // If running in bootloader mode, the bootloader 
    // will send a CAN message about the reset once the
