...
void canRxUpdate(void)
{
        ...
        else if (msg_header.Bus == CAN1)
        {
            switch(msg_header.ExtId)
            {
                ...
                case ID_INVB_ERR_2:
                    ...
                    break;
                case ID_IMD_STATUS_RAW:
                    can_data.imd_status_raw.period = msg_data_a->imd_status_raw.period;
                    can_data.imd_status_raw.duty_cycle = msg_data_a->imd_status_raw.duty_cycle;
                    can_data.imd_status_raw.stale = 0;
                    can_data.imd_status_raw.last_rx = sched.os_ticks;
                    break;
				...
				return_fault_control(msg_data_a->return_fault_control.id);
                    break;
                case ID_DAQ_COMMAND_MAIN_MODULE_VCAN:
                    can_data.daq_command_MAIN_MODULE_VCAN.daq_command = msg_data_a->daq_command_MAIN_MODULE_VCAN.daq_command;
                    daq_command_MAIN_MODULE_VCAN_CALLBACK(&msg_header);
                    break;
                default:
                    __asm__("nop");
            }
    ...
    CHECK_STALE(can_data.INVB_ERR_2.stale,
                ...
                UP_INVB_ERR_2);
    CHECK_STALE(can_data.imd_status_raw.stale,
                sched.os_ticks, can_data.imd_status_raw.last_rx,
                UP_IMD_STATUS_RAW);
...
bool initCANFilter()
{
    ...
    CAN1->sFilterRegister[10].FR2 = (ID_IMD_STATUS_RAW << 3) | 4;
    CAN1->FA1R |= (1 << 11);    // configure bank 11
    CAN1->sFilterRegister[11].FR1 = (ID_FAULT_SYNC_PDU << 3) | 4;
    CAN1->sFilterRegister[11].FR2 = (ID_FAULT_SYNC_DASHBOARD << 3) | 4;
    CAN1->FA1R |= (1 << 12);    // configure bank 12
    CAN1->sFilterRegister[12].FR1 = (ID_FAULT_SYNC_A_BOX << 3) | 4;
    CAN1->sFilterRegister[12].FR2 = (ID_FAULT_SYNC_TORQUE_VECTOR << 3) | 4;
    CAN1->FA1R |= (1 << 13);    // configure bank 13
    CAN1->sFilterRegister[13].FR1 = (ID_FAULT_SYNC_TEST_NODE << 3) | 4;
    CAN1->sFilterRegister[13].FR2 = (ID_SET_FAULT << 3) | 4;
    CAN1->FA1R |= (1 << 14);    // configure bank 14
    CAN1->sFilterRegister[14].FR1 = (ID_RETURN_FAULT_CONTROL << 3) | 4;
    CAN1->sFilterRegister[14].FR2 = (ID_DAQ_COMMAND_MAIN_MODULE_VCAN << 3) | 4;
    /* END AUTO FILTER */
