...
void carPeriodic() {
    ...
    if (car.state == CAR_STATE_FATAL) {
        ...
        if (car.start_btn_debounced) {
            ...
            // Enforce range
            temp_t_req.torque_left =
                CLAMP(temp_t_req.torque_left, MIN_DRIVER_TORQUE_REQUEST, MAX_DRIVER_TORQUE_REQUEST);
            temp_t_req.torque_right =
                CLAMP(temp_t_req.torque_right, MIN_DRIVER_TORQUE_REQUEST, MAX_DRIVER_TORQUE_REQUEST);
...
void monitorSDCPeriodic() {
    ...
    *(nodes + index) = stat;
    ...
    if (index == SDC_MUX_HIGH_IDX) {
        ...
        SEND_SDC_STATUS(sdc_mux.main_stat,
                        sdc_mux.c_stop_stat,
                        sdc_mux.inertia_stat,
                        sdc_mux.bots_stat,
                        sdc_mux.bspd_stat,
                        sdc_mux.bms_stat,
                        sdc_mux.imd_stat,
