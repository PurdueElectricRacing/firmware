...
GPIOInitConfig_t gpio_config[] = {
    ...
    */
    ...
    /* Wheel Speeds */
    GPIO_INIT_AF(MOTOR_R_WS_GPIO_Port,
                 MOTOR_R_WS_Pin,
                 MOTOR_R_WS_AF,
                 GPIO_OUTPUT_HIGH_SPEED,
                 GPIO_OUTPUT_OPEN_DRAIN,
                 GPIO_INPUT_PULL_DOWN),
    GPIO_INIT_AF(MOTOR_L_WS_GPIO_Port,
                 MOTOR_L_WS_Pin,
                 MOTOR_L_WS_AF,
                 GPIO_OUTPUT_HIGH_SPEED,
                 GPIO_OUTPUT_OPEN_DRAIN,
...
dma_init_t adc_dma_config =
    ADC1_DMA_CONT_CONFIG((uint32_t)&adc_readings, sizeof(adc_readings) / sizeof(adc_readings.shock_l), 0b01);
...
void heartBeatLED(void) {
    ...
    if (trig) {
        ...
        SEND_MAIN_MODULE_CAN_STATS(can_stats.can_peripheral_stats[CAN1_IDX].tx_of,
                                   can_stats.can_peripheral_stats[CAN2_IDX].tx_of,
                                   ...
                                   can_stats.can_peripheral_stats[CAN2_IDX].rx_overrun);
    }
    trig = !trig;
}
...
void can2Relaycan1(CAN_TypeDef* can_h) {
    can_peripheral_stats_t* rx_stats =
        (can_h == CAN1) ? (&can_stats.can_peripheral_stats[CAN1_IDX]) : (&can_stats.can_peripheral_stats[CAN2_IDX]);
