    ...
    if(qReceive(&q_rx_can, &msg_header) == SUCCESS_G)
    {
        ...
        switch(msg_header.ExtId)
        {
            ...
            case ID_RETURN_FAULT_CONTROL:
                ...
                break;
            case ID_DAQ_COMMAND_PDU_VCAN:
                can_data.daq_command_PDU_VCAN.daq_command = msg_data_a->daq_command_PDU_VCAN.daq_command;
                daq_command_PDU_VCAN_CALLBACK(&msg_header);
                break;
            default:
                __asm__("nop");
        }
