...
static void calculate_cooling_periodic() {
    ...
    // Enable if above 30C
    // TODO handle stale
    if (!can_data.max_cell_temp.stale && (can_data.max_cell_temp.max_temp >= BATT_COOLING_ENABLE_TEMP * 10)) {
    ...
    if (!can_data.rear_motor_temps.stale && ((can_data.rear_motor_temps.left_mot_temp >= MOTOR_COOLING_ENABLE_TEMP || can_data.rear_motor_temps.right_mot_temp >= MOTOR_COOLING_ENABLE_TEMP))) {
    ...
    if (!can_data.rear_motor_temps.stale && ((can_data.rear_motor_temps.left_mot_temp >= MOTOR_COOLING_MAX_TEMP || can_data.rear_motor_temps.right_mot_temp >= MOTOR_COOLING_MAX_TEMP) || ((can_data.rear_motor_temps.left_mot_temp >= MOTOR_COOLING_ENABLE_TEMP || can_data.rear_motor_temps.right_mot_temp >= MOTOR_COOLING_ENABLE_TEMP) && not_moving))) {
        cr.fan2_status = true;
