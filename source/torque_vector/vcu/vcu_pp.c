...
void vcu_pp(fVCU_struct *fVCU, xVCU_struct *xVCU, GPS_Handle_t *gps, BMI088_Handle_t *bmi, State_Estimate_t *state_estimate)
{
    ...
    xVCU->AG_RAW[0] = (state_estimate->accel_x); /* Incoming data is m/s^2 */
    xVCU->AG_RAW[1] = (state_estimate->accel_y); /* Incoming data is m/s^2 */
    xVCU->AG_RAW[2] = (state_estimate->accel_z); /* Incoming data is m/s^2 */
