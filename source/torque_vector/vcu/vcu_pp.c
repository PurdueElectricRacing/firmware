...
void vcu_pp(fVCU_struct* fVCU, xVCU_struct* xVCU, GPS_Handle_t* GPS) {
    ...
    xVCU->MT_RAW = (MAX((can_data.INVA_TEMPS.AMK_MotorTemp), (can_data.INVB_TEMPS.AMK_MotorTemp))
                    * 0.1); /* incoming is 10*motor temp*/
    xVCU->CT_RAW = (MAX((can_data.INVA_TEMPS.AMK_IGBTTemp), (can_data.INVB_TEMPS.AMK_IGBTTemp))
                    * 0.1); /* incoming is 10*controller igbt temp*/
    xVCU->IT_RAW = (MAX((can_data.INVA_TEMPS.AMK_InverterTemp), (can_data.INVB_TEMPS.AMK_InverterTemp))
                    * 0.1); /* incoming is 10*inverter cold plate temp*/
    xVCU->MC_RAW = (MAX((can_data.INVA_CRIT.AMK_DisplayOverloadMotor), (can_data.INVB_CRIT.AMK_DisplayOverloadMotor))
                    * 0.1); /* incoming data is 10*% */
    xVCU->IC_RAW =
        (MAX((can_data.INVA_CRIT.AMK_DisplayOverloadInverter), (can_data.INVB_CRIT.AMK_DisplayOverloadInverter))
         * 0.1); /* incoming data is 10*% */
