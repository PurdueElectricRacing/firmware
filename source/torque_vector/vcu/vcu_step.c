...
void vcu_step(const pVCU_struct* p, const fVCU_struct* f, const xVCU_struct* x, yVCU_struct* y) {
    ...
    if (f->VCU_CFLAG == p->VCU_CFLAG_CS) {
        if (roundf(((((y->VS_permit_buffer[0] + y->VS_permit_buffer[1]) + y->VS_permit_buffer[2]) + y->VS_permit_buffer[3]) + y->VS_permit_buffer[4]) / 5.0F) != 0.0F) {
            y->VCU_mode = 3.0F;
        } else {
            y->VCU_mode = 0.0F;
        }
    } else if (roundf(((((y->VT_permit_buffer[0] + y->VT_permit_buffer[1]) + y->VT_permit_buffer[2]) + y->VT_permit_buffer[3]) + y->VT_permit_buffer[4]) / 5.0F) != 0.0F) {
        y->VCU_mode = 4.0F;
    } else if (roundf(((((y->PT_permit_buffer[0] + y->PT_permit_buffer[1]) + y->PT_permit_buffer[2]) + y->PT_permit_buffer[3]) + y->PT_permit_buffer[4]) / 5.0F) != 0.0F) {
        y->VCU_mode = 2.0F;
    } else {
        y->VCU_mode = 1.0F;
    }
    ...
    if (y->VCU_mode == 4.0F) {
        ...
        if (y->VT_mode == 1.0F) {
            ...
            if ((Xqk >= p->TV_ST_brkpt[0]) && (Xqk <= p->TV_ST_brkpt[52]) && (Yqk >= p->TV_GS_brkpt[0]) && (Yqk <= p->TV_GS_brkpt[50])) {
                float f9;
                float qx1;
                float qx2;
                int b_high_i;
            ...
            y->TV_delta_torque = fmaxf(fminf((y->TV_AV_ref * p->TV_PI - y->AV_CF[2]) * y->TV_PP_CF * p->ht[1],
                                             y->TO_PT[0] * p->MAX_r),
                                       -y->TO_PT[0] * p->MAX_r);
