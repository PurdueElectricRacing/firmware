...
void vcu_step(const pVCU_struct* p, const fVCU_struct* f, const xVCU_struct* x, yVCU_struct* y) {
    ...
    if (f->VCU_CFLAG == p->VCU_CFLAG_CS) {
        if (roundf(
                ((((y->VS_permit_buffer[0] + y->VS_permit_buffer[1]) + y->VS_permit_buffer[2]) + y->VS_permit_buffer[3])
    ...
    } else if (roundf(((((y->VT_permit_buffer[0] + y->VT_permit_buffer[1]) + y->VT_permit_buffer[2])
                        + y->VT_permit_buffer[3])
                       + y->VT_permit_buffer[4])
                      / 5.0F)
               != 0.0F) {
        y->VCU_mode = 4.0F;
    } else if (roundf(((((y->PT_permit_buffer[0] + y->PT_permit_buffer[1]) + y->PT_permit_buffer[2])
    ...
    for (d_k = 0; d_k < 3; d_k++) {
        y->AV_CF[d_k] =
            fmaxf(fminf((p->R[d_k] * f1 + p->R[d_k + 3] * f2) + p->R[d_k + 6] * f3, p->AV_ub[d_k]), p->AV_lb[d_k]);
    }
    for (i6 = 0; i6 < 9; i6++) {
    ...
    for (f_k = 0; f_k < 3; f_k++) {
        y->AG_CF[f_k] =
            fmaxf(fminf((p->R[f_k] * f4 + p->R[f_k + 3] * f5) + p->R[f_k + 6] * f6, p->AG_ub[f_k]), p->AG_lb[f_k]);
    }
    ...
    y->VS_MAX_SR_CF = fmaxf(fminf(x->VS_MAX_SR_RAW, p->VS_MAX_SR_ub), p->VS_MAX_SR_lb);
    y->zero_current_counter = (y->zero_current_counter + 1.0F) * (float)(y->IB_CF == 0.0F);
        ...
        if ((xi <= d_varargin_1[505]) && (xi >= d_varargin_1[0])) {
            ...
            if (r == 0.0F) {
                ...
                if (capacity_used_tmp == varargin_2[low_i]) {
                    ...
                    capacity_used = (1.0F - r) * capacity_used_tmp + r * varargin_2[low_i];
                }
            }
        }
    ...
    if (y->VCU_mode >= 2.0F) {
        ...
        fv2[1] = 0.0F;
        varargin_1_idx_1 =
            p->MAX_TORQUE_NOM * interp1(f_p, fv1, fmaxf(fminf(y->BT_CF, p->bT_derating_zero_T), p->bT_derating_full_T));
        varargin_1_idx_2 =
            p->MAX_TORQUE_NOM * interp1(g_p, fv2, fmaxf(fminf(y->VB_CF, p->Vb_derating_zero_T), p->Vb_derating_full_T));
        ex_tmp =
            p->MAX_TORQUE_NOM * interp1(e_p, fv, fmaxf(fminf(y->IT_CF, p->iT_derating_zero_T), p->iT_derating_full_T));
        ex = ex_tmp;
    ...
    if (y->VCU_mode == 4.0F) {
        ...
        if (y->VT_mode == 1.0F) {
            ...
            if ((Xqk >= p->TV_ST_brkpt[0]) && (Xqk <= p->TV_ST_brkpt[52]) && (Yqk >= p->TV_GS_brkpt[0])
                && (Yqk <= p->TV_GS_brkpt[50])) {
                float f9;
                float qx1;
                float qx2;
                int b_high_i;
            ...
            y->TV_delta_torque =
                fmaxf(fminf((y->TV_AV_ref * p->TV_PI - y->AV_CF[2]) * y->TV_PP_CF * p->ht[1], y->TO_PT[0] * p->MAX_r),
                      -y->TO_PT[0] * p->MAX_r);
