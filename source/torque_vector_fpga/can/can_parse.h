...
#define NODE_NAME "Torque_Vector_fpga"
...
#define SEND_FAULT_SYNC_TORQUE_VECTOR_FPGA(queue, idx_, latched_) do {\
        CanMsgTypeDef_t msg = {.Bus=CAN1, .ExtId=ID_FAULT_SYNC_TORQUE_VECTOR_FPGA, .DLC=DLC_FAULT_SYNC_TORQUE_VECTOR_FPGA, .IDE=1};\
        CanParsedData_t* data_a = (CanParsedData_t *) &msg.Data;\
        data_a->fault_sync_torque_vector_fpga.idx = idx_;\
        data_a->fault_sync_torque_vector_fpga.latched = latched_;\
        qSendToBack(&queue, &msg);\
    } while(0)
